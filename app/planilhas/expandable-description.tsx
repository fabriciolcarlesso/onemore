...
export function ExpandableDescription({ text }: { text: string }) {
  ...
  return <div className="mt-2">
    <div className={`relative ${expanded ? "" : "max-h-18 overflow-hidden"}`}>
    ...
    <button type="button" aria-label={expanded ? "Recolher descrição da planilha" : "Expandir descrição da planilha"} title={expanded ? "Recolher" : "Expandir"} onClick={() => setExpanded((current) => !current)} className="flex h-5 w-full items-center justify-center text-slate-400 transition hover:text-slate-700"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={`size-4 transition-transform ${expanded ? "rotate-180" : ""}`}><path strokeLinecap="round" strokeLinejoin="round" d="m6 9 6 6 6-6" /></svg></button>
  </div>;
}
