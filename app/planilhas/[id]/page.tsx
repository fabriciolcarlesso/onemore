...
import { db } from "@/db";
import { workoutExercises, workoutGroups, workoutSheets, workouts } from "@/db/schema";
...
export default async function WorkoutSheetDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const workoutIds = sheetWorkouts.map((workout) => workout.id);
  const groups = workoutIds.length ? await db.select({ id: workoutGroups.id, workoutId: workoutGroups.workoutId, restSeconds: workoutGroups.restSeconds }).from(workoutGroups).where(inArray(workoutGroups.workoutId, workoutIds)) : [];
  const groupIds = groups.map((group) => group.id);
  const exerciseRows = groupIds.length ? await db.select({ groupId: workoutExercises.groupId, sets: workoutExercises.sets, repetitions: workoutExercises.repetitions }).from(workoutExercises).where(inArray(workoutExercises.groupId, groupIds)) : [];
  const workoutStats = new Map(workoutIds.map((workoutId) => {
    const workoutGroupsForWorkout = groups.filter((group) => group.workoutId === workoutId);
    const groupIdsForWorkout = new Set(workoutGroupsForWorkout.map((group) => group.id));
    const exercisesForWorkout = exerciseRows.filter((exercise) => groupIdsForWorkout.has(exercise.groupId));
    const workSeconds = exercisesForWorkout.reduce((total, exercise) => total + exercise.sets * exercise.repetitions * 3, 0);
    const restSeconds = workoutGroupsForWorkout.reduce((total, group) => {
      const groupExercises = exercisesForWorkout.filter((exercise) => exercise.groupId === group.id);
      const rounds = groupExercises.reduce((max, exercise) => Math.max(max, exercise.sets), 0);
      return total + (group.restSeconds ?? 0) * Math.max(0, rounds - 1);
    }, 0);
    return [workoutId, { exerciseCount: exercisesForWorkout.length, estimatedMinutes: Math.ceil((workSeconds + restSeconds) / 60) }] as const;
  }));
  ...
  return <DashboardShell user={user} signOut={signOut} activePage="sheets"><div className="pb-20">
    ...
    {sheetWorkouts.length ? <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{sheetWorkouts.map((workout) => <WorkoutCard key={workout.id} workout={{ ...workout, ...(workoutStats.get(workout.id) ?? { exerciseCount: 0, estimatedMinutes: 0 }) }} />)}</ul> : <div className="rounded-2xl border border-slate-200 bg-white px-6 py-16 text-center text-sm text-slate-400">Nenhum treino nesta planilha ainda.</div>}
