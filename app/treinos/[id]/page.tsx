...
export default async function WorkoutDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <DashboardShell user={user} signOut={signOut} activePage={workout.sheetId ? "sheets" : "workouts"}>
    <div>
      <Link href={backHref} className="text-sm text-slate-500 hover:text-slate-950">← {backLabel}</Link>
      {user.role !== "aluno" ? <Link href={`/treinos/${workout.id}/editar`} className="ml-4 inline-block rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">Editar treino</Link> : null}
