    ...
    <form action={formAction} className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8" autoComplete="off" noValidate>
      ...
      <div className="mt-6 space-y-4">
        ...
        <div>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white focus-within:border-slate-950 focus-within:ring-2 focus-within:ring-slate-950/10"><div className="flex items-center px-3 pb-0 pt-3"><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={toggleBold} title="Aplicar negrito" aria-label="Aplicar negrito" className="-ml-[10px] -mt-1 flex size-8 cursor-pointer items-center justify-center rounded-md text-base font-bold text-slate-400 underline decoration-1 underline-offset-2 transition hover:bg-slate-100 hover:text-slate-700">B</button></div><textarea ref={descriptionRef} id="exercise-description" name="description" rows={6} placeholder="Inclua instruções ou observações" value={fields.description} onChange={(event) => setFields((current) => ({ ...current, description: event.target.value }))} className="w-full resize-y border-0 bg-transparent px-4 pb-3 pt-2 text-base outline-none" /></div>
        </div>
      </div>
