...
export function ExerciseImage({ name, slug }: { name: string; slug: string }) {
  ...
  return (
    <div className="group relative aspect-[3/4] w-full overflow-hidden bg-slate-100">
      <Image key={`${gender}-${imageNumber}`} src={`/images/exercises/${slug}-${gender}-${imageNumber}.png`} alt={`Demonstração ${genderLabel} de ${name}, imagem ${imageNumber} de 3`} fill className="animate-in object-cover transition-opacity duration-300 ease-in-out" />
