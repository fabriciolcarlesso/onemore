...
export default async function DoWorkoutPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <WorkoutPlayer emailPending={user.role === "aluno" && !user.emailVerifiedAt} workoutId={workout.id} name={workout.name} steps={steps} gender={user.preferredTeacher === "julieta" ? "feminino" : "masculino"} />;
}
